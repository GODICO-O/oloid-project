...
bool MenuLayer_init_hook(void* self) {
    LOGI("Oloid: Hook Aktif! Manifestasi MenuLayer via Unified Core! 🚀");
    return MenuLayer_init_original(self);
}

extern "C" JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM* vm, void* reserved) {
    LOGI("Oloid Core: Memulai proses Hooking Aktif (Unified)... 🕵️‍♂️");
    ...
    if (cocos_handle) {
        ...
        if (target) {
            ...
            LOGI("Oloid: HOOKING SUKSES! ✅ Satu file untuk kedaulatan KLOS.");
        }
